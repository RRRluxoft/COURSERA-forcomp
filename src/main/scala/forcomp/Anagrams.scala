...
object Anagrams {
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    occurrences.map(e => (for(i <- 1 to e._2) yield (e._1, i)).toList)
      .foldRight(List[Occurrences](Nil)) ((xs, ys) => ys ++ (for(x <- xs; y <- ys) yield x :: y))
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    val (s1, s2) = x.partition(a => y.exists(b => (a._1 == b._1)))
    val diffs = for((a, b) <- s1 zip y if (a._2 != b._2)) yield (a._1, a._2 - b._2)
    (s2 ++ diffs).sorted
  }
